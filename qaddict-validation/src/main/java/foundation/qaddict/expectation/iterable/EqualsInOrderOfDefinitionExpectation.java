...
public record EqualsInOrderOfDefinitionExpectation<D>(Collection<Expectation<? super D>> expectations) implements Expectation<Iterable<D>> {

    @Override
    public EvaluationNode evaluate(Iterable<D> data) {
        var iterator = data.iterator();
        if(expectations.stream()
                .map(expectation -> iterator.hasNext() && expectation.evaluate(iterator.next()).result())
                .reduce((l, r) -> l && r)
                .orElse(true)
                && !iterator.hasNext())
